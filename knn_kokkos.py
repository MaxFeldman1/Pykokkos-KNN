...
# -----------------------------
# kernel: k = b
# In hblk_col_body, Ldst is flushed to INF before each hblk iteration and we
# insert exactly b = k distances (one per im_h). Each im_h maps directly to
# its slot — no eviction ever occurs — so the O(k) worst-find scan is removed.
# -----------------------------
@pk.workunit
def knn_pipeline_kernel_keqb(team_member: pk.TeamMember,
    ...
    for hblk_i in range(1, l):
        ...
        # k = b: Ldst flushed to INF before this iteration, exactly b = k
        # distances inserted (one per im_h), so im_h is the direct slot index.
        def hblk_col_body(jm: int):
            j: pk.int32 = jm + b * hblk_i
            im_h: pk.int32 = 0
            for im_h in range(b):
...
def run_knn_pipeline(N, m, d, k, b, X, Xn, Dloc, Gdst, Gidx, Ldst, Lidx):
    ...
    Uses the k=b optimised kernel when k == b, otherwise the general kernel.
    """
